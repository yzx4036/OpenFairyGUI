...
import { escapeCSharpString } from './naming.js';
...
async function loadTpl(name: string): Promise<string> {
    if (!_cache) {
        ...
        const results = await Promise.all(files.map(async (f) => {
            _cache![f] = await readFile(join(TPL_DIR, `${f}.tpl`), 'utf-8');
        }));
    }
    return _cache[name];
}
...
function renderMemberAssignment(
    member: EtCodegenComponent['members'][number],
...
function assertEntity(component: EtCodegenComponent): asserts component is EtCodegenComponent & { entityTypeName: string } {
    if (!component.entityTypeName) {
        throw new Error(`Component ${component.packageName}/${component.componentName} does not generate an ET entity.`);
    }
}
